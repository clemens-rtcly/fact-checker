...
def _ist_uhrzeit(toks: list["_Tok"], j: int, n: int) -> bool:
    """„14:30 Uhr" ist keine Verhältnisangabe.

    Die Ziffernform ist zwischen Spielstand und Uhrzeit nicht entscheidbar
    („2:0" gegen „14:30"), das nachgestellte „Uhr" ist im Deutschen aber
    fast immer da. Zusätzlich gilt ein zweistelliger Minutenwert über 12
    als Zeitindiz — 0:0 bis 12:59 bleiben damit Spielstände, was in
    Lokalnachrichten die häufigere Lesart ist.
    """
    if j + 1 < n and toks[j + 1].low.rstrip(".") in {"uhr"}:
        return True
    zweite = toks[j].text
    return len(zweite) == 2 and zweite[0] != "0" and float(toks[j].num) > 12


# Ausgeschriebene Ziffernketten: „zwei-eins-zwei-C-D" ist die gesprochene
# Form von „212CD". Ohne eigene Behandlung zerfällt sie in drei kleine
# Zahlen (2, 1, 2), die im Artikel irgendwo anders ebenfalls vorkommen —
# und erzeugen dort einen Zahlkonflikt gegen eine völlig unbeteiligte
# Stelle. Die Ziffernform wird von `_typbezeichnung` bereits unterdrückt;
# hier kommt die Transkriptseite dazu.
_ZIFFERWORT = {
    "null": "0", "eins": "1", "ein": "1", "zwei": "2", "drei": "3",
    "vier": "4", "fünf": "5", "sechs": "6", "sieben": "7", "acht": "8",
    "neun": "9",
}
# Mindestens drei Glieder, jedes entweder Ziffernwort oder Einzelbuchstabe,
# und mindestens ein Ziffernwort darunter. Zwei Glieder wären zu wenig
# („S-Bahn"), gewöhnliche Komposita scheitern schon am Gliedtest
# („Baden-Württemberg").
_KETTE = re.compile(
    r"\b(?:[A-Za-zÄÖÜäöüß]+)(?:-(?:[A-Za-zÄÖÜäöüß]+)){2,}\b")


def _kette_aufloesen(stueck: str) -> str | None:
    """„zwei-eins-zwei-C-D" -> „212CD", sonst None."""
    glieder = stueck.split("-")
    if len(glieder) < 3:
        return None
    raus, ziffern = [], 0
    for g in glieder:
        low = g.lower()
        if low in _ZIFFERWORT:
            raus.append(_ZIFFERWORT[low])
            ziffern += 1
        elif len(g) == 1 and g.isalpha():
            raus.append(g.upper())
        else:
            return None
    return "".join(raus) if ziffern else None


def _kettenspannen(text: str) -> list[tuple[int, int]]:
    """Zeichenbereiche, die zu einer Ziffernkette gehören."""
    return [(m.start(), m.end()) for m in _KETTE.finditer(text)
            if _kette_aufloesen(m.group(0))]


def _typbezeichnung(text: str, ende: int) -> bool:
    """Folgen der Zahl ohne Leerzeichen Buchstaben? Dann Typ, keine Menge.

    „212CD", „A7", „5G" sind Bezeichner. Sie als Zahl zu führen erzeugt
    einen Zahlkonflikt gegen jede beliebige andere Zahl im Artikel, und
    inhaltlich prüft sie ohnehin die Kürzelprüfung in `identifier.py`,
    die auf exakter Gleichheit besteht statt auf numerischer Nähe.
    """
    nach = text[ende:ende + 1]
    return bool(nach) and nach.isalpha()


def _spanne_ok(toks: list["_Tok"], a: int, b: int, n: int,
               text: str, v1: float) -> bool:
    """Darf aus `NUM bis NUM` eine Spanne werden?

    Bewusst eng. Wo eine der Bedingungen scheitert, bleibt es beim
    bisherigen Verhalten (zwei unabhängige Zahlen) — der Zweig kann also
    nichts verschlechtern, nur weniger verbessern.

    Ausgeschlossen sind:

      Datumsspannen  „vom 8. bis 10. Juli". Der Ordnungspunkt steht nicht
                     im Token, geprüft wird das Zeichen dahinter. Sonst
                     kollidiert die Spanne mit der Datumserkennung.
      Uhrzeiten      „von 8 bis 10 Uhr".
      Skala/Einheit  „1,6 bis 1,8 Millionen", „acht bis zehn Jahre". Die
                     Skala gilt für beide Endpunkte und müsste
                     mitgerechnet werden; das ist ein eigener Fall.
      Nicht-Ganze    „1,6 bis 1,8" ohne Skala. Die Normform `8-10` ist auf
                     ganze Zahlen ausgelegt.
      Absteigend     „zehn bis acht" ist keine Spanne, sondern ein
                     Aufzählungsrest.
    """
    v2 = float(toks[b].num)
    if not (float(v1).is_integer() and v2.is_integer()) or v2 <= v1:
        return False
    for t_i in (a, b):
        if text[toks[t_i].end:toks[t_i].end + 1] == ".":
            return False                      # Ordnungszahl -> Datumsspanne
    if b + 1 < n:
        folge = toks[b + 1].low.rstrip(".")
        if folge in _SCALE_WORDS or folge in _MONEY_WORDS \
                or folge in _PERCENT_WORDS or folge == "uhr" \
                or _unit_of_token(folge) is not None:
            return False
    return True
...
def extract_entities(text: str) -> list[Entity]:
    ...
    while i < n:
        ...
        if t.num is None:
            i += 1
            continue
        # Glied einer ausgeschriebenen Ziffernkette („zwei-eins-zwei-C-D"):
        # keine Menge, sondern Teil eines Bezeichners. Den Abgleich
        # übernimmt die Kürzelprüfung in `identifier.py`, die die Kette
        # zur Ziffernform auflöst und auf exakter Gleichheit besteht.
        if any(a <= t.start < b for a, b in ketten):
            i += 1
            continue
        ...
        # Verhältnis: NUM "zu" NUM oder NUM ":" NUM
        # Der Doppelpunkt steht nicht in `_TOKEN_RE` und wird deshalb nicht
        # zum Token — geprüft wird der Rohtext zwischen beiden Zahlen.
        # Ohne diesen Zweig ist die Normalisierung einseitig: Das Transkript
        # sagt „null zu null" -> `0:0`, der Artikel schreibt „0:0" und
        # liefert zwei unabhängige Nullen. Derselbe Fall wie
        # „dreißigjähriger" gegen „30-Jährige".
        verh = None
        if etype is None and scale == 1 and j < n:
            if toks[j].low == "zu" and j + 1 < n and toks[j + 1].num is not None:
                verh = j + 1
            elif (toks[j].num is not None and toks[j].is_digit and t.is_digit
                  and text[toks[j - 1].end:toks[j].start] == ":"
                  and not _ist_uhrzeit(toks, j, n)):
                verh = j
        if verh is not None:
            k = verh
            v2 = float(toks[k].num)
        ...
        # Spanne: NUM "bis" NUM
        # Eine Spanne ist eine Entität, keine zwei. „acht bis zehn Teams"
        # zerfiel bisher in zwei kleine Zahlen — beide unterhalb der
        # Ankerschwelle und im Artikel selten eindeutig, weshalb der Satz,
        # der sie enthält, nicht als Fundstelle ergänzt wurde. Als Spanne
        # `8-10` ist der Wert distinktiv genug für einen Anker, im Artikel
        # praktisch immer eindeutig, und beide Schreibweisen („acht bis
        # zehn", „8 bis 10") bekommen dieselbe Normform. Nebeneffekt: Eine
        # Verfälschung zu „acht bis zwölf" wird ein sauberer Zahlkonflikt,
        # statt sich in zwei unabhängigen Zahlen zu verstecken.
        # „zwischen 30 und 45" ist dieselbe Spanne wie „von 30 bis 45" —
        # ohne diesen Zweig war die Normalisierung wieder einseitig, und
        # ein Artikel mit „zwischen" gegen ein Transkript mit „bis" ergab
        # einen Zahlkonflikt, wo keiner ist. „und" gilt NUR nach
        # „zwischen": sonst würde jede Aufzählung („2 und 3 Prozent") zur
        # Spanne.
        spannenwort = j < n and (
            toks[j].low == "bis"
            or (toks[j].low == "und" and start_tok > 0
                and toks[start_tok - 1].low == "zwischen"))
        if (etype is None and scale == 1 and j + 1 < n
                and spannenwort and toks[j + 1].num is not None
                and _spanne_ok(toks, start_tok, j + 1, n, text, value)):
            k = j + 1
            end_tok = k
            surface = text[toks[start_tok].start:toks[end_tok].end]
            norm = f"{int(value)}-{int(float(toks[k].num))}"
            ents.append(Entity("zahl", surface, norm, norm,
                               toks[start_tok].start, toks[end_tok].end))
            i = k + 1
            continue
        ...
        # Typbezeichnung statt Menge: „212CD", „A7", „5G". Nur wenn die
        # Zahl allein steht (keine Skala, keine Einheit, kein Typ) —
        # „5 Grad" bleibt eine Messung.
        if (etype is None and scale == 1 and unit is None
                and t.is_digit and start_tok == end_tok
                and _typbezeichnung(text, toks[end_tok].end)):
            i = end_tok + 1
            continue
